...
namespace Drupal\Tests\oe_graphql\Kernel\DataProducer\Routing;
...
/**
 * Test class for the RouteEntityRevision data producer.
 */
class RouteEntityRevisionTest extends GraphQLTestBase {
  ...
  public function testRouteEntity(): void {
    ...
    $result = $this->executeDataProducer('oe_graphql_route_entity_revision', [
      'url' => $url,
      'revision_id' => $this->published_node->getRevisionId(),
    ]);
    $this->assertEquals('Test Event rev 2', $result->label());
    ...
    $this->assertNull($this->executeDataProducer('oe_graphql_route_entity_revision', [
      'url' => 'not_a_url',
    ]));
